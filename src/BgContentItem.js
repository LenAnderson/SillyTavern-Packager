...
export class BgContentItem extends ContentItem {
    /**@type {{name:string}} */ content = null;




    constructor(path) {
        super({ name:path }, 'name', 'name', 'bg');
        ...
        if (!this.dom) {
            const c = document.createElement('div'); {
                ...
                const img = document.createElement('img'); {
                    img.classList.add('stpkg--bg');
                    img.src = `/thumbnail?type=bg&file=${this.content.name}`;
                    c.append(img);
                }
